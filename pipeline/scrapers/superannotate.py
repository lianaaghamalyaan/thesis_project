"""SuperAnnotate scraper — Lever public API, Armenia filter."""
...
LEVER_URL = "https://api.lever.co/v0/postings/superannotate"
HEADERS = {"User-Agent": "ThesisResearch/1.0 (Armenian IT curriculum alignment; academic use)"}
RAW_CSV = RAW_DIR / "superannotate_jobs_raw.csv"
...
def scrape(seen_urls: set | None = None) -> list[dict]:
    ...
    print(f"  [superannotate] {len(armenia_jobs)} Armenia, {len(new_jobs)} new")
    ...
    print(f"  [superannotate] done — {count} new rows appended")
    return records
